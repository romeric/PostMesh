...
{
    //! CONVERT ALL IMPORTED SURFACES TO BSPLINE SURFACES: http://dev.opencascade.org/doc/refman/html/class_geom_convert.html
...
{
    //! GETS PARAMETER BOUNDS OF SURFACE
    ...
    for (auto iface=0; iface<this->mesh_faces.rows(); ++iface)
    {
        ...
        if (this->projection_criteria(iface)==1)
        {
            ...
            for (UInteger isurface=0; isurface<this->geometry_surfaces.size(); ++isurface)
            {
                ...
                {
                    // STORE ID OF CURVES
                    this->dirichlet_faces(index_face,3) = isurface;
                    // RE-ASSIGN
